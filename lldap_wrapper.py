...
def create_user(uid, email, displayname=None, firstname=None, lastname=None) -> tuple[bool, str]:
    ...
    if reset_type == "authelia":
        ...
        try:
            ...
            return (
                False,
                f"User created but Authelia reset failed (network error). {type(e).__name__}: {e}",
            )
        except Exception as e:  # noqa: BLE001
            return (
                False,
                f"User created but Authelia reset failed (unexpected error). {type(e).__name__}: {e}",
            )
        ...
        try:
            ...
            return (
                False,
                f"User created but lldap reset failed (network error). {type(e).__name__}: {e}",
            )
        except Exception as e:  # noqa: BLE001
            return (
                False,
                f"User created but lldap reset failed (unexpected error). {type(e).__name__}: {e}",
            )
